...
namespace Mediachase.Commerce.Workflow.Activities.OrderGroupActivities
{
    public class UpdateTotalsVNextActivity : OrderGroupActivityBase
    {
        ...
        {
            var formPayments = orderForm.Payments.ToArray();
            orderForm.AuthorizedPaymentTotal = GetPaymentAmount(formPayments, TransactionType.Authorization);
            orderForm.CapturedPaymentTotal = GetPaymentAmount(formPayments, TransactionType.Sale);
            orderForm.CapturedPaymentTotal += GetPaymentAmount(formPayments, TransactionType.Capture);
            orderForm.CapturedPaymentTotal -= GetPaymentAmount(formPayments, TransactionType.Credit);
        }

        private decimal GetPaymentAmount(IEnumerable<Payment> payments, TransactionType type)
        {
            return base.GetResultingPaymentsByTransactionType(payments, type).Where(x => PaymentStatusManager.GetPaymentStatus(x) == PaymentStatus.Processed).Sum(y => y.Amount);
        }
