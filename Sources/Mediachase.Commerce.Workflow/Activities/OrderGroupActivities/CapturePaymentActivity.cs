...
namespace Mediachase.Commerce.Workflow.Activities
{
    ...
    public class CapturePaymentActivity : OrderGroupActivityBase
    {
        ...
        {
            var retVal = Shipment.SubTotal + Shipment.ShippingTotal;
            ...
            foreach (var lineItem in Shipment.GetShipmentLineItems(Shipment))
            {
                ...
                var itemsPriceExcTax = lineItem.PlacedPrice - _billingCurrency.Round((lineItem.OrderLevelDiscountAmount + lineItem.LineItemDiscountAmount) / lineItem.Quantity);
                decimal itemTax = taxRate > 0 ? _billingCurrency.Percentage(itemsPriceExcTax, taxRate) : 0;
                retVal += itemTax * quantity;
            }
        ...
        private void DoCapture(Payment authorizePayment, decimal amount)
        {
            ...
            Payment payment = OrderGroup.OrderForms[0].Payments.AddNew(paymentType);
