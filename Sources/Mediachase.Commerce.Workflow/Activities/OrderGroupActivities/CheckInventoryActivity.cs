...
namespace Mediachase.Commerce.Workflow.Activities
{
    public class CheckInventoryActivity : OrderGroupActivityBase
    {
            ...
            foreach (LineItem lineItem in validLineItems)
            {
                ...
                else
                {
                    ...
                    if (delta != 0)
                    {
                        ...
                        ChangeShipmentsLineItemQuantity(lineItem, delta);
        ...
        {
            var orderForm = OrderGroup.OrderForms.FirstOrDefault();
            if (orderForm != null)
            {
                int lineItemIndex = orderForm.LineItems.IndexOf(lineItem);
                var allShipmentContainsLineItem = orderForm.Shipments.Where(x => Shipment.GetShipmentLineItems(x).Contains(lineItem));
        ...
        private void ChangeShipmentsLineItemQuantity(LineItem lineItem, decimal delta)
        {
            var orderForm = OrderGroup.OrderForms.FirstOrDefault();
            if (orderForm != null)
            {
